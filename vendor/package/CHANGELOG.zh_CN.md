...
- **`iLink-App-Id` / `iLink-App-ClientVersion` 请求头**：所有 API 请求（GET 与 POST）均携带 `iLink-App-Id`（取自 `package.json#ilink_appid`）和 `iLink-App-ClientVersion`（semver 按 `0x00MMNNPP` 编码为 uint32）。
- **`apiGetFetch()`**：新增 GET 请求封装函数，用于扫码登录与状态轮询，替代原先的裸 `fetch()` 调用，保证请求头一致注入。
- **CDN `full_url` 支持**：`MediaItem` 及上传响应新增 `full_url` / `upload_full_url` 字段；服务端返回完整 URL 时直接使用，无需客户端拼接。回退行为由 `ENABLE_CDN_URL_FALLBACK` 控制。
- **扫码登录 IDC 重定向**：新增对 `scaned_but_redirect` 状态的处理，轮询时自动切换至 `redirect_host`，无需重启登录流程。
- **`ilink_appid: "bot"`** 字段写入 `package.json`。

### 变更

- **扫码登录固定 Base URL**：`fetchQRCode` 与状态轮询统一使用固定地址 `https://ilinkai.weixin.qq.com`，登录前不再要求配置 `apiBaseUrl`。
- **扫码轮询容错**：`pollQRStatus` 遇到网络/网关错误（如 Cloudflare 524）时不再抛出，改为返回 `{ status: "wait" }` 并继续重试。
- **多账号会话隔离配置项**：文档更新，配置键由 `agents.mode per-channel-per-peer` 改为 `session.dmScope per-account-channel-peer`。
- **兼容性错误提示**：不再硬编码插件版本号，升级引导改为 `npx @tencent-weixin/openclaw-weixin-cli install`。
- **MEDIA 指令系统提示**：增加对模型的约束说明——`MEDIA:` 标签必须单独占一行。

### 移除

- 从 `package.json` 删除 **`peerDependencies`**（`openclaw >= 2026.3.22`）。
