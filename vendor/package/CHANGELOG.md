...
- **`notifyStart` / `notifyStop`**: Channel client now calls `notifyStart` on startup and `notifyStop` on graceful shutdown (new `stopAccount` lifecycle hook).
- **`iLink-App-Id` and `iLink-App-ClientVersion` headers**: All API requests now carry app identity and version headers sourced from `package.json` (`ilink_appid` field; version encoded as uint32 `0x00MMNNPP`).
- **`apiGetFetch`**: Shared GET request wrapper with optional timeout and common headers, used by QR code fetch and QR status polling.
- **CDN server-side full URL support**: `upload_full_url` (upload) and `full_url` (download) fields from API responses are used directly; client-side URL construction falls back via `ENABLE_CDN_URL_FALLBACK`.
- **QR login IDC redirect**: New `scaned_but_redirect` status — polling host switches dynamically to `redirect_host` without restarting the login flow.

### Changed

- **QR login base URL**: QR code fetch and status polling now always use the fixed URL `https://ilinkai.weixin.qq.com`; `apiBaseUrl` config is no longer required to initiate a login.
- **QR status polling resilience**: Network/gateway errors (e.g. Cloudflare 524 timeouts) are now treated as `wait` and retried instead of propagating an exception.
- **Compat install guidance**: Error message updated to point to `npx @tencent-weixin/openclaw-weixin-cli install` (removed reference to the deprecated `1.x` legacy track).
- **`package.json`**: Added `ilink_appid: "bot"` field; removed `peerDependencies` for `openclaw`.
